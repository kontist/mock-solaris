...
export const getTaxIdentifications = async (personId) => JSON.parse(
  (await redisClient.getAsync(`${process.env.MOCKSOLARIS_REDIS_PREFIX}:taxIdentifications:${personId}`)) || '[]'
);

export const saveTaxIdentifications = async (personId, data) => redisClient.setAsync(
  `${process.env.MOCKSOLARIS_REDIS_PREFIX}:taxIdentifications:${personId}`,
  JSON.stringify(data, undefined, 2)
);

export const getMobileNumber = async (personId) => JSON.parse(
  await redisClient.getAsync(`${process.env.MOCKSOLARIS_REDIS_PREFIX}:mobileNumber:${personId}`)
);

export const saveMobileNumber = async (personId, data) => redisClient.setAsync(
  `${process.env.MOCKSOLARIS_REDIS_PREFIX}:mobileNumber:${personId}`,
  JSON.stringify(data, undefined, 2)
);

export const deleteMobileNumber = async (personId) =>
  redisClient.delAsync(`${process.env.MOCKSOLARIS_REDIS_PREFIX}:mobileNumber:${personId}`);
...
export const getAllIdentifications = () => {
  return getAllPersons()
    .then((persons) => {
      return _.flattenDeep(persons.map((person) => {
...
export const findPersonByAccountId = (accountId) =>
  findPersonByAccountField((person) => (
...
export const getWebhookByType = async (type) => (await getWebhooks()).find((webhook) => webhook.event_type === type);

export const getIdentificationWebhook = () => getWebhookByType('IDENTIFICATION');
export const getBookingWebhook = () => getWebhookByType('BOOKING');
export const getSepaDirectDebitReturnWebhook = () => getWebhookByType('SEPA_DIRECT_DEBIT_RETURN');

export const getSepaDirectDebitReturns = async () => {
  const sepaDirectDebitReturns = JSON.parse(
    (await redisClient.getAsync(`${process.env.MOCKSOLARIS_REDIS_PREFIX}:sepa_direct_debit_returns`)) || '[]'
  );
...
export const getPersonBookings = (person) => {
  return (person.transactions || [])
    .map(fillMissingCurrencyForLegacyBooking);
};
