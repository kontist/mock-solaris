...
export const patchIdentification = async (req, res) => {
  ...
  const person = await getPerson(personId) || { identifications: {}, transactions: [] };
  person.identifications[identificationId] = person.identifications[identificationId] || {};
...
export const showPersonIdentifications = (req, res) => {
  ...
  return getPerson(personId)
    .then((_person) => {
      ...
      const personIdentifications = Object.values(person.identifications)
